...
# --- DYNAMIC SUMMARY BOXES ---
        ...
        if 'Case Last Modified Date' in df.columns:
             closed_in_period_df = df[
                ...
                (df['Status'].isin(CLOSED_STATUSES)) &
                (df['Product Line'].isin(productline))
            ].copy()
        ...
        # Find latest date available in dataset
        latest_date_in_data = max(
            pd.to_datetime(df['Opened Date'], errors='coerce').max(),
            pd.to_datetime(df['Case Last Modified Date'], errors='coerce').max()
        )
